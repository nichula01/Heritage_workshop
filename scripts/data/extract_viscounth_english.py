...
def main():
    missing = [p for p in [TRAIN_DIR, DESC_DIR] if not p.exists()]
    if missing:
        paths = "\n".join(f"- {p}" for p in missing)
        raise SystemExit(
            "Expected VISCOUNTH folders were not found. Download/unpack VISCOUNTH locally first:\n"
            f"{paths}\n"
            "See docs/dataset_setup.md for the expected data/raw layout."
        )
