...
OUT_CSV = Path("results/internvl25_2b_routed_aer_v3/predictions.csv")
        ...
        for i, row in enumerate(rows_in, start=1):
            ...
            try:
                if route == "visual":
                    ...
                    # fallback extractor only if model gives empty/useless answer
                    ...
                    if (not pred.strip() or pred.strip().lower() in {
                        "unknown", "not specified", "not provided", "i do not know"
                    }) and extracted:
                        pred = extracted
                        answer_source = "extractor_fallback"

                else:
                    # mixed -> extractor first
                    extracted = extract_answer(
                        question=question,
                        evidence_text=evidence_text,
                        route=route
